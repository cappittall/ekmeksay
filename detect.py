...
def writetocsv(counter):
    if counter[3] > 0:  ## At least should be 1 bread in 1 min
        ...
        with open('/home/mendel/besas_' + counter[0][:10].replace('/', '') + '.csv', 'a') as ff:
            wr = writer(ff)
...
def overlay(layout, objs, trdata, axis, roi, inference_time, inference_rate, trend, sifirla ):
    ...
    # Info
    line = 'Satirda bulunan:%d,  Inf. time %.2f ms (%.2f fps)(Inf rate: %.2f Track time: %.2f)' % (len(trdata), inference_time * 1000, 1.0 / inference_time, inference_rate, trend * 1000)
    y = oy2 #- 1.7 * font_size
...
def render_gen(args):
    ...
    ## initiliaze tracker
    args.track_thresh = args.threshold
    args.track_buffer = 10
    args.match_thresh = 0.3
    
    #bjectOfTracker = ObjectTracker('sort')
    #racker = objectOfTracker.trackerObject.mot_tracker
    tracker = Sort(max_age=13, min_hits=0, iou_threshold=.30)
